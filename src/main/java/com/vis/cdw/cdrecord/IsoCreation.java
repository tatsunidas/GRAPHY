...
public class IsoCreation {
	...
	public static void main(String[] args) {
		...
		// function
		sampleCreation.insertFile(new File("DICOM-CD-TEST"));//"test directory"
		...
		sampleCreation.finalizeDisc(new File("tmp/test.iso"));
	}
	...
	// https://github.com/danveloper/provisioning-gradle-plugin/blob/
	...
	public ISO9660Config setIsoConfig(String volumeId) {
		...
		// The standard says you are not allowed more than 8 levels, unless you enable
		// RockRidge support
		...
		iso9660config.forceDotDelimiter(false);//false is default,but get warn breaks ISO 9660 conformance.
		try {
	...
	public JolietConfig setJolietConfig(String volumeId) {
		...
		jolietConfig.forceDotDelimiter(false);//false is default,but get warn breaks Joliet conformance.
		return jolietConfig;
	}
