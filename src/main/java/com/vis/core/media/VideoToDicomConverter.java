...
public class VideoToDicomConverter {
	...
	public static void convertVideo(File videoFile, File tempDir, int seriesNumber, int instanceNumber, Modality m,
		...
		// 2. 全フレームのピクセルデータをメモリ上に展開して結合する
		int bytesPerPixel = isColor ? 3 : 1;
		int frameSize = w * h * bytesPerPixel;
		long totalPixelBytes = (long) frameSize * frames;
		
		// メモリ限界（約2GB）を超える巨大動画のガード
		if (totalPixelBytes > Integer.MAX_VALUE - 8) {
			throw new OutOfMemoryError("Video file is too large to convert entirely in memory.");
		}

		byte[] allPixels = new byte[(int) totalPixelBytes];
		int offset = 0;

		for (int k = 0; k < frames; k++) {
			imp.setSlice(k + 1); // ディスクから1フレーム読み込み
			if (!isColor) {
				ImageProcessor ip = imp.getProcessor().convertToByte(true);
				byte[] b = (byte[]) ip.getPixels();
				System.arraycopy(b, 0, allPixels, offset, frameSize);
				offset += frameSize;
			} else {
				...
				int pIndex = 0;
				for (int p = 0; p < w * h; p++) {
					allPixels[offset + pIndex++] = r[p];
					allPixels[offset + pIndex++] = g[p];
					allPixels[offset + pIndex++] = b[p];
				}
				offset += frameSize;
			}
		}
		...
		// 3. ピクセルデータを正規のDICOMタグ(OB形式)として一括セット！
		dcmHeader.setBytes(Tag.PixelData, VR.OB, allPixels);

		// 4. dcm4cheに完璧なフォーマット（Implicit VR）でファイル出力させる
...
//	 */
//	private static void appendPixelDataStream(File outputFile, ImagePlus imp, int w, int h, int frames, boolean isColor)
//			throws Exception {
//		int bytesPerPixel = isColor ? 3 : 1;
//		int frameSize = w * h * bytesPerPixel;
//
...
//	 */
//	private static void cleanupFailedFile(File file) {
