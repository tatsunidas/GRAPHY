...
public class MaskRoiPanel extends JPanel {
    ...
    public MaskRoiPanel(RoiObj attachedRoi, com.vis.core.view.D2.ui.glasses.Praparat ownerPraparat, String seriesLabel, int currentSlice, MaskRoiPanelListener listener) {
    ...
    private void initUI(String seriesLabel, int currentSlice) {
        ...
        txtCustomRange.addActionListener(e -> {
            if (listener != null) listener.onRangeChanged(MaskRoiPanel.this);
        });
        ...
        ((javax.swing.text.AbstractDocument) txtCustomRange.getDocument()).setDocumentFilter(new javax.swing.text.DocumentFilter() {
            @Override
 ...
 // --- 修正: モードに応じた対象スライス（ZCTインデックス）を取得するメソッド ---
    ...
    public int[] getTargetSliceIndices(Praparat ownerPraparat) {
        ...
        java.util.concurrent.ConcurrentHashMap<Integer, com.vis.core.view.D2.ui.glasses.SlideGlass> slides = ownerPraparat.getAllSlides();
    ...
    /**
     * カンマ区切りやハイフン指定（例: "1-5", "1,3,5"）のテキストをパースし、
     * 対象となるZインデックス（0ベース）のSetを返す。
