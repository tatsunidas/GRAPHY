...
//        add(mainSplit, BorderLayout.CENTER);
	...
	private void updateMaskRoiListForCurrentSeries() {
        if (maskRoiListPanel == null || currentActivePraparat == null) return;
 ...
 // =========================================================================================
    ...
    private void onExecuteClicked(ActionEvent e) {
        ...
        if (destPath == null || destPath.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Please select an output destination directory.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        List<HashMap<String,String>> targets = getTargetSeriesList();
        if (targets.isEmpty()) {
            JOptionPane.showMessageDialog(this, "No series selected. Please check at least one series in the tree.", "Warning", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        currentWorker = new SwingWorker<Void, String>() {
            ...
            protected Void doInBackground() throws Exception {
                ...
                File tempDir = new File(System.getProperty("java.io.tmpdir"), "GraphyAnonTemp_" + System.currentTimeMillis());
            ...
            protected void done() {
                ...
                try {
                    if (isCancelled()) {
                        ...
                        JOptionPane.showMessageDialog(PixelAnonymizerPanel.this, "Process was canceled.", "Canceled", JOptionPane.WARNING_MESSAGE);
                    } else {
                        get(); 
                        progressBar.setValue(100);
                        progressBar.setString("Completed");
                        JOptionPane.showMessageDialog(PixelAnonymizerPanel.this, "Pixel and Attribute Anonymization Completed Successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
                    }
                } catch (Exception ex) {
    ...
    private void processPixelMaskingToTemp(String seriesUid, File tempDir, SwingWorker<?,?> worker) throws Exception {
        ...
        for (RoiObj roi : tempRois) {
            if (!seriesUid.equals(roi.getProperty(ContextKey.SeriesInstanceUID.name()))) continue;
        ...
        if (prap.isMultiFrame()) {
            ...
            if (isMpeg) {
                ...
                com.vis.dicom.image.DicomImage dcm = new com.vis.dicom.dcm4cheImpl.DicomImageChe(filePath, header, fmi, tsuid);
                ...
                writer.openStream(header, fmi, tempFile.getAbsolutePath(), tsuid.uid(), 
                                  numFrames, dcm.getBitsAllocated(), dcm.getSamples(), 
                                  dcm.getWidth(), dcm.getHeight());
            ...
            for (int i = 0; i < filePaths.size(); i++) {
                ...
                SwingUtilities.invokeLater(() -> {
                    if (progressBar != null) {
                        ...
                        progressBar.setIndeterminate(false);
                        progressBar.setValue(percent);
                        progressBar.setString(String.format("Masking Files: %d / %d (%d%%)", currentFile, totalFiles, percent));
                    }
                });
                ...
                if (rois != null && !rois.isEmpty()) {
                    ...
                    dcmImg.setPixelData(0, ip.getWidth(), ip.getHeight(), dcmImg.getSamples(), dcmImg.getBitsAllocated(), newPixels);
                    com.vis.dicom.DicomWriter writer = com.vis.dicom.DicomWriter.newDicomWriter();
                    writer.writeDicomImage(dcmImg.getHeader(), fmi, new File(tempDir, sop + ".dcm").getAbsolutePath(), false);
                } else {
                    java.nio.file.Files.copy(new java.io.File(filePath).toPath(), new java.io.File(tempDir, sop + ".dcm").toPath(), java.nio.file.StandardCopyOption.REPLACE_EXISTING);
                }
            }
        }
    }
...
//    }
 ...
 // =========================================================================
    ...
    private void processMpegMaskingToTemp(com.vis.dicom.DicomObject header, com.vis.dicom.DicomObject fmi, 
        ...
        if (!filter.isEmpty()) {
            try {
                ...
                ProcessBuilder pb = new ProcessBuilder(
                        javeFfmpegPath, "-y", "-i", tempIn.getAbsolutePath(),
                        "-vf", filter,
                        "-c:v", "libx264", 
                        "-preset", "medium", 
                        "-crf", "18", // ★ 高画質を維持するオプション
        ...
        for (Map.Entry<RoiObj, java.util.List<Integer>> entry : framesPerRoi.entrySet()) {
            ...
            if (ijRoi != null) {
                ...
                for (int i = 1; i < frames.size(); i++) {
                    ...
                    if (curr != prev + 1) {
                        if (start == prev) enableConditions.add(String.format("eq(n\\,%d)", start));
                        else enableConditions.add(String.format("between(n\\,%d\\,%d)", start, prev));
                        start = curr;
                    }
                    prev = curr;
                }
    ...
    private int[] calculateTargetZctIndices(Praparat prap, RoiObj roi) {
        ...
        if (slides == null) return new int[0];

        if (mode == 0) { // All
            int[] indices = new int[slides.size()];
            int i = 0;
            for (Integer index : slides.keySet()) indices[i++] = index;
    ...
    private java.util.Set<Integer> parseCustomRangeToZ(String text) {
        ...
        if (text == null || text.trim().isEmpty()) return zSet;
            ...
            try {
                if (part.contains("-")) {
                    ...
                    if (range.length == 2) {
                        ...
                        for (int z = min; z <= max; z++) zSet.add(z);
                    }
                } else {
                    int val = Integer.parseInt(part) - 1;
                    if (val >= 0) zSet.add(val);
                }
...
//            	System.out.println("Listen !");
	...
	private void addMaskRoiPanel(RoiObj targetRoi, Praparat pp, int mode, String custumRangeTxt) {
        ...
        MaskRoiPanel roiPanel = new MaskRoiPanel(targetRoi, pp, seriesLabel, currentSlice, new MaskRoiPanel.MaskRoiPanelListener() {
            @Override
