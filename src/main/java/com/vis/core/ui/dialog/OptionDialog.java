 ...
 * OptionDialog with one string field and two numeric fields:
 * <pre>
 *	public class Generic_Dialog_Example implements PlugIn {
 *	  static String title="Example";
 *	  static int width=512,height=512;
 *	  public void run(String arg) {
 *		OptionDialog gd = new OptionDialog("New Image");
...
*/
public class OptionDialog extends JDialog implements ActionListener, DocumentListener,
FocusListener, ItemListener, KeyListener, AdjustmentListener, WindowListener {
	...
	protected Vector<JComponent> numberField, stringField, checkbox, choice, slider;
	protected Vector<ButtonGroup> radioButtonGroups;
...
//	private Vector<JPanel> imagePanels;
	protected static OptionDialog instance;
	private boolean firstPaint = true;
	private boolean fontSizeSet;
	private boolean optionsRecorded;	 // have dialogListeners been called to record options?
	private JLabel lastLabelAdded;
	private int[] windowIDs;
	private String[] windowTitles;


	/** Creates a new OptionDialog with the specified title. Uses the current image
	...
	/** Creates a new OptionDialog using the specified title and parent frame. */
	public OptionDialog(String title, JFrame parent) {
   ...
   public void addNumericField(String label, double defaultValue, int digits, int columns, String units) {
		...
		JLabel fieldLabel = makeLabel(label2);
...
//		if (Platform.isWindows()) columns -= 2;
		...
		if (Double.isNaN(defaultValue))
			defaultString = "";
		JTextField tf = newTextField(defaultString, columns);
		if (Platform.isLinux()) tf.setBackground(Color.white);
...
//		boolean custom = customInsets;
		...
		JTextField tf = newTextField(defaultText, columns);
		if (Platform.isLinux()) tf.setBackground(Color.white);
...
//	}
	...
	public void addDirectoryField(String label, String defaultPath, int columns) {
		...
		if (GraphicsEnvironment.isHeadless())
			return;
		JTextField text = (JTextField)stringField.lastElement();
	 ...
	 * in Fiji's OptionDialogPlus class.
	 */
	...
	*/
	 ...
	 * method in Fiji's OptionDialogPlus class.
	 * @param label	 the label
	...
	*/
	 ...
	 * {@link OptionDialog#addChoice(String, String[], String)} method.
	 * Usage example:
	 * <pre>
	 ...
	 * 
	 * OptionDialog gd = new OptionDialog("Select AutoThresholder Method");
	 ...
	 * (string-based) {@link OptionDialog#addChoice(String, String[], String)}
...
//		this.pfr = pfr;
	 ...
	 * Note that a OptionDialog can have only one PreviewCheckbox.
	 */
...
//		int startCBIndex = cbIndex;
	...
	*/
	public void addRadioButtonGroup(String label, String[] items, int rows, int columns, String defaultItem) {
		...
		for (int i=0; i<n; i++) {
			JCheckBox cb = new JCheckBox(items[i],items[i].equals(defaultItem));
			group.add(cb);
			cb.addItemListener(this);
			panel.add(cb);
		}
		...
		radioButtonGroups.addElement(group);
   ...
   */
   public void addChoice(String label, String[] items, String defaultItem) {
		...
		add(fieldLabel, c);
		JComboBox<String> thisChoice = new JComboBox<>();
	...
	*/
	public void addTextAreas(String text1, String text2, int rows, int columns) {
		...
		if (Platform.isLinux()) textArea1.setBackground(Color.white);
		...
		if (text2!=null) {
			...
			if (Platform.isLinux()) textArea2.setBackground(Color.white);
...
//		GUI.fixScrollbar(s);
		...
		JTextField tf = newTextField(IJ.d2s(defaultValue/scale,digits),columns);
		//if (Platform.isLinux()) tf.setBackground(Color.white);
		tf.addActionListener(this);
		tf.getDocument().addDocumentListener(this);
		tf.addFocusListener(this);
	...
	private JTextField newTextField(String txt, int columns) {
//		if (Platform.isLinux())
...
//	}
	...
	*/
	...
	/** Make this a "Yes No Cancel" dialog with custom labels. Here is an example:
		<pre>
		OptionDialog gd = new OptionDialog("YesNoCancel Demo");
   ...
   public String getNextString() {
	...
	public String getNextChoice() {
		...
		JComboBox<String> jComboBox = (JComboBox<String>)(choice.elementAt(choiceIndex));
		JComboBox<String> thisChoice = jComboBox;
	...
	public int getNextChoiceIndex() {
		...
		if (macro) {
			String label = (String)labels.get((Object)thisChoice);
			String oldItem = (String) thisChoice.getSelectedItem();
		...
		if (recorderOn) {
			...
			if (!(smartRecording&&index==defaultIndex)) {
				...
				if (!(item.equals("*None*")&&getTitle().equals("Merge Channels")))
					recordOption(thisChoice, (String)thisChoice.getSelectedItem());
			}
		}
		choiceIndex++;
		return index;
	...
	public String getNextRadioButton() {
		if (radioButtonGroups==null)
			return null;
		ButtonGroup cg = (ButtonGroup)(radioButtonGroups.elementAt(radioButtonIndex));
		radioButtonIndex++;
		JCheckBox checkbox = (JCheckBox) cg.getSelection();
		String item = "null";
		if (checkbox!=null)
	...
	public void showDialog() {
		if (macro) {
			...
			if (Platform.isWindows() || Prefs.dialogCancelButtonOnRight) {
				buttons.add(okay);
			...
			resetCounters();
			revalidate();
			setVisible(true);  //except for NonBlockingOptionDialog, returns after 'dispose' by OK or Cancel
		}
	}
...
//				((DialogListener)dialogListeners.elementAt(0)).dialogItemChanged(this,null);
	...
	*/
	public JButton[] getButtons() {
		return new JButton[] {okay, cancel, no, help};
	}
	...
	*/
	public void previewRunning(boolean isRunning) {
		if (previewCheckbox!=null) {
			...
			if (Platform.isMac()) repaint();	//workaround OSX 10.4 refresh bug
		}
	}
...
//			showHelp();
	...
	public void keyPressed(KeyEvent e) {
		...
		if ((component instanceof Scrollbar) && (keyCode==KeyEvent.VK_LEFT||keyCode==KeyEvent.VK_RIGHT)) {
			JScrollBar sb = (JScrollBar)component;
		...
		if (keyCode==KeyEvent.VK_ENTER && textArea1==null && okay!=null && okay.isEnabled()) {
			...
			dispose();
			IJ.resetEscape();
		} else if (keyCode==KeyEvent.VK_W && (e.getModifiersEx()&Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx())!=0) {
			wasCanceled = true;
			dispose();
		}
	}
	...
	private void notifyListeners() {
		...
		for (int i=0; everythingOk && i<dialogListeners.size(); i++) {
			try {
				resetCounters();
//				if (this instanceof NonBlockingOptionDialog)
...
//					everythingOk = false;		  // disable further listeners if false (invalid parameters) returned
		...
		resetCounters();
		boolean workaroundOSXbug = Platform.isMac() && okay!=null && !okay.isEnabled() && everythingOk;
	...
	public void paint(Graphics g) {
		super.paint(g);
		if (firstPaint && Platform.isMac() && IJ.isJava18()) { // fix for incompletely drawn dialogs on Macs
	...
	static class TextDropTarget extends DropTargetAdapter {
		...
		public TextDropTarget(JTextField text) {
			this.text = text;
		}
	...
	private class BrowseButtonListener implements ActionListener {
		...
		public BrowseButtonListener(JTextField textField, String mode) {
...
//	private class TrimmedTextField extends JTextField {
//	
//		/**
//		 * 
//		 */
