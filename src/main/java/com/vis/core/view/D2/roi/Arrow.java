...
public class Arrow extends com.vis.core.view.D2.roi.Line {
...
//            {updateFullWindow = false; imp.draw();}
    ...
    private ShapeRoi getShapeRoi() {
        ...
        ShapeRoi sroi = new ShapeRoi(arrow, getSlideGlass());
        Shape outlineShape = stroke.createStrokedShape(arrow);
        sroi.or(new ShapeRoi(outlineShape, getSlideGlass()));
        return sroi;
    }
	...
	public void drawPixels(ImageProcessor ip) {
		...
		if (outline) {
			...
			//keep Roi
			ij.gui.Roi copy = new RoiConverter().convert2Roi(shapeRoi);
			ip.fill(copy);
			if (doubleHeaded) {
				ij.gui.Roi copy2 = new RoiConverter().convert2Roi(shapeRoi2);
				ip.fill(copy2);
			}
		}
	}
