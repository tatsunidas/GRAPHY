...
public class IntersectVolume extends LocalizerPoster {
	...
	public IntersectVolume(Vector3d row, Vector3d column, Vector3d tlhc, Vector3d voxelSpacing, Vector3d dimensions) {
	...
	@Override
	public List<Point2D> getOutlineOnLocalizerForThisGeometry(Vector3d row, Vector3d column, Vector3d tlhc,
			Vector3d voxelSpacing, double sliceThickness, Vector3d dimensions) {

		Vector3d[] corners = getCornersOfSourceCubeInSourceSpace(row, column, tlhc, voxelSpacing, sliceThickness,
				dimensions);
		for (int i = 0; i < 8; ++i) {
			...
			// Edges with one Z value +ve (or 0) and the other -ve (or 0) cross (or touch)
			// the localizer
			// plane
