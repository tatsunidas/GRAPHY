...
public class GeometryOfSlice {
	...
	// row spacing (between centers of adjacent rows), then column spacing, then
	// slice
	 ...
	 *
	 * @param row            the direction of the row as X, Y and Z components
	 *                       (direction cosines, unit vector) LPH+
	 * @param column         the direction of the column as X, Y and Z components
	 *                       (direction cosines, unit vector) LPH+
	 * @param tlhc           the position of the top left-hand corner of the slice
	 *                       as a point (X, Y and Z) LPH+
	 * @param voxelSpacing   the row and column spacing and, if a volume, the slice
	 *                       interval (spacing between the centers of parallel
	 *                       slices) in mm
	 * @param sliceThickness the slice thickness in mm
	 * @param dimensions     the row and column dimensions and 1 for the third
	 *                       dimension
	 */
	public GeometryOfSlice(Vector3d row, Vector3d column, Vector3d tlhc, Vector3d voxelSpacing, double sliceThickness,
			Vector3d dimensions) {
		this.row = row;
		this.column = column;
		this.tlhc = tlhc;
...
//		this.dimensions = new Vector3d(new double[] {numRow,numColumn,1});
	 ...
	 *
	 * @return the direction of the row as X, Y and Z components (direction cosines,
	 *         unit vector) LPH+
	 */
	 ...
	 *
	 * @return the direction of the normal to the plane of the slices, as X, Y and Z
	 *         components (direction cosines, unit vector) LPH+
	 */
	 ...
	 * toUpperCase() to produce valid CodeString for PatientOrientation).
	 *
	...
	public static String getOrientation(Vector3d orientation, boolean quadruped) {
		...
		if (orientation != null) {
			String orientationX = orientation.x < 0 ? (quadruped ? "Rt" : "R") : (quadruped ? "Le" : "L"); // NON-NLS
			String orientationY = orientation.y < 0 ? (quadruped ? "V" : "A") : (quadruped ? "D" : "P"); // NON-NLS
			String orientationZ = orientation.z < 0 ? (quadruped ? "Cd" : "F") : (quadruped ? "Cr" : "H"); // NON-NLS
	 ...
	 *
	 * @param quadruped true if subject is a quadruped rather than a biped
	 * @return a string rendering of the row orientation, more than one letter if
	 *         oblique to the orthogonal axes, or empty string (not null) if fails
	 */
	 ...
	 *
	 * @param quadruped true if subject is a quadruped rather than a biped
	 * @return a string rendering of the column orientation, more than one letter if
	 *         oblique to the orthogonal axes, or empty string (not null) if fails
	 */
