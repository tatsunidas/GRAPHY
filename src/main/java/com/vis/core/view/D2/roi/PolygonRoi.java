...
public class PolygonRoi extends RoiObj {
	...
	/**
	 * Creates a new polygon or polyline ROI from x and y coordinate arrays. Type
	 * must be Roi.POLYGON, Roi.FREEROI, Roi.TRACED_ROI, Roi.POLYLINE, Roi.FREELINE
	 * or Roi.ANGLE.
	 */
	public PolygonRoi(int[] xPoints, int[] yPoints, int nPoints, int type, SlideGlass slide) {
		super(0, 0, 0, 0, 0,slide);
		init1(nPoints, type);
		xp = xPoints;
		yp = yPoints;
		if (type != RoiType.TRACED_ROI.id()) {
			xp = new int[nPoints];
			yp = new int[nPoints];
	...
	/**
	 * Creates a new polygon or polyline ROI from float x and y arrays. Type must be
	 * Roi.POLYGON, Roi.FREEROI, Roi.POLYLINE, Roi.FREELINE or Roi.ANGLE.
	 */
	public PolygonRoi(float[] xPoints, float[] yPoints, int nPoints, int type,SlideGlass slide) {
		super(0, 0, 0, 0, 0, slide);
		init1(nPoints, type);
	 ...
	 * Roi.POLYGON, Roi.FREEROI, Roi.POLYLINE, Roi.FREELINE or Roi.ANGLE.
	 */
	public PolygonRoi(float[] xPoints, float[] yPoints, int type,SlideGlass slide) {
		this(xPoints, yPoints, xPoints.length, type,slide);
	}
...
//		userCreated = true;
	 ...
	 * Roi.POLYGON, Roi.FREEROI, Roi.TRACED_ROI, Roi.POLYLINE, Roi.FREELINE or
	 * Roi.ANGLE.
	 */
	public PolygonRoi(Polygon p, int type, SlideGlass slide) {
		this(p.xpoints, p.ypoints, p.npoints, type, slide);
	}
	 ...
	 * Roi.ANGLE.
	 */
	public PolygonRoi(FloatPolygon p, int type, SlideGlass slide) {
		this(p.xpoints, p.ypoints, p.npoints, type, slide);
	}
...
//		}
	...
	public void drawPixels(ImageProcessor ip) {
		...
		if (xSpline != null) {
			...
			if (type == RoiType.POLYGON.id() || type == RoiType.FREEROI.id() || type == RoiType.TRACED_ROI.id()) // close the shape
...
//		}
		...
		if (nPoints > 1) {
			...
			if (type != RoiType.ANGLE.id() && slide != null) {
				/*
...
//			imp.draw(xmin - margin, ymin - margin, (xmax - xmin) + margin * 2, (ymax - ymin) + margin * 2);
	...
	void finishPolygon() {
		...
		oldWidth = width;
		oldHeight = height;
		if (Recorder.record && userCreated && (type == RoiType.POLYGON.id() || type == RoiType.POLYLINE.id() || type == RoiType.ANGLE.id()
				|| (type == RoiType.POINT.id() && Recorder.scriptMode() && nPoints == 3)))
	...
	public void exitConstructingMode() {
		if (type == RoiType.POLYLINE.id() && state == CONSTRUCTING) {
			addOffset();
			finishPolygon();
		}
	}
	...
	protected void moveHandle(int sx, int sy) {
		...
		if (xSpline != null) {
			...
			if (!subPixelResolution() || (type == RoiType.POINT.id() && nPoints == 1))
				resetBoundingRect();
			if (type == RoiType.POINT.id() && width == 0 && height == 0) {
				width = 1;
				height = 1;
			}
...
//		}
	...
	void addHandle(double ox, double oy, SlideGlass sg) {
		...
		modState = NO_MODS;
		RoiObj prevRoi = RoiObj.getPreviousRoi();
		if (prevRoi != null)
...
//		}
	...
	/**
	 * Fits a spline, but leaves the Roi unchanged. The arrays supplied (which may
	 * be null) are reused if the number of spline points remains unchanged.
	 * 
	 * @return Arrays of image x coordinates and image y coordinates
	 */
...
//		double length = getUncalibratedLength();
	...
	/**
	 * Creates a spline fitted polygon with (roughly) one pixel segment lengths and
	 * sets it as the Roi shape. It can be retrieved using the getFloatPolygon()
	 * method.
	 */
	 ...
	 * 'segmentLength', and returns arrays of x & y coordinates. Since the full
	 * length is not necessarily an integer multiple of 'segmentLength', the
	 * segmentLength actually used is returned as array element [2][0]. If the
	 * ImagePlus is given, calibrated coordinates are used, based on its
	 * calibration, and 'segmentLength' should be in calibrated units. Returns null
	 * if an input array is null, or the number of points is 0
	 */
	static float[][] getEquidistantPoints(float[] xpoints, float[] ypoints, int npoints, double segmentLength,
		...
		for (int i = 1; i < npoints; i++) {
			...
			while ((distanceOverNextWrite >= 0.0 || i == npoints - 1) && pointsWritten < npOut) { // we have to write a
				...
				// IJ.log("i="+i+" n="+pointsWritten+"/"+npOut+"
	...
	/**
	 * With segmented selections, ignore first mouse up and finalize when user
	...
	/**
	 * Returns whether the center of pixel (x,y) is contained in the Roi. The
	 * position of a pixel center determines whether a pixel is selected. Note the
	 * ImageJ convention of 0.5 pixel shift between outline and pixel centers, i.e.,
	 * pixel (0,0) is enclosed by the rectangle spanned between (0,0) and (1,1).
	 ...
	 * border with the lower (higher) y. This convention is opposite to that of the
	 * java.awt.Shape class. In x, the offset is chosen slightly below 0.5 to reduce
	...
	public boolean contains(int x, int y) {
		...
		if (xSpline != null) {
			FloatPolygon poly = new FloatPolygon(xSpline, ySpline, splinePoints);
			return poly.contains((float)(x - getXBase() + 0.4999f), (float)(y - getYBase() + 0.5f));
		} else if (xpf != null) {
			FloatPolygon poly = new FloatPolygon(xpf, ypf, nPoints);
			return poly.contains((float)(x - getXBase() + 0.4999), (float)(y - getYBase() + 0.5));
		} else {
			Polygon poly = new Polygon(xp, yp, nPoints);
			return poly.contains((float)(x - this.x + 0.4999), (float)(y - this.y + 0.5));
		}
	}
	...
	public boolean containsPoint(double x, double y) {
		...
		if (xSpline != null) {
			FloatPolygon poly = new FloatPolygon(xSpline, ySpline, splinePoints);
			return poly.contains((float)(x - getXBase() + 1e-7), (float)(y - getYBase() + 1e-10));
		} else if (xpf != null) {
			FloatPolygon poly = new FloatPolygon(xpf, ypf, nPoints);
			return poly.contains((float)(x - getXBase() + 1e-7), (float)(y - getYBase() + 1e-10));
		} else {
...
//			pf.setPolygon(xp, yp, nPoints);
	...
	/**
	 * Returns the length of this line selection after smoothing using a 3-point
	 * running average.
	 */
	...
	/**
	 * Returns the perimeter of this ROI after smoothing using a 3-point running
	 * average.
	 */
	...
	/**
	 * Returns the perimeter length of ROIs created using the wand tool and the
	 * particle analyzer. The algorithm counts pixels in straight edges as 1 and
	 * pixels in corners as sqrt(2). It does this by calculating the total length of
	 * the ROI boundary and subtracting 2-sqrt(2) for each non-adjacent corner. For
	 * example, a 1x1 pixel ROI has a boundary length of 4 and 2 non-adjacent edges
	 * so the perimeter is 4-2*(2-sqrt(2)). A 2x2 pixel ROI has a boundary length of
	 * 8 and 4 non-adjacent edges so the perimeter is 8-4*(2-sqrt(2)). Note that
	 * this code can currently create inconsistent legths depending on the starting
	 * position.
	 */
	double getTracedPerimeter(ImagePlus imp) {
		...
		/*
		 * Alternative code leading to slightly different results: It does this by
		 * calculating the total length of the ROI boundary and subtracting 1-sqrt(2)/2
		 * for each corner. For example, a 1x1 pixel ROI has a boundary length of 4 and
		 * and 4 corners so the perimeter is 4-4*(1-sqrt(2)/2) = 2*sqrt(2). A 2x2 pixel
		 * ROI has a boundary length of 8 and 4 corners so the perimeter is
		 * 8-4*(1-sqrt(2)/2) = 4 + 2*sqrt(2). /* int x0 = xp[nPoints-2]; int y0 =
		 * yp[nPoints-2]; int x1 = xp[nPoints-1]; int y1 = yp[nPoints-1]; int sumdx = 0;
		 * int sumdy = 0; int nCorners = 0; for (int i=0; i<nPoints; i++) { int x2 =
		 * xp[i]; int y2 = yp[i]; //The following is the cross product r01 x r12 =
		 * (x1-x0)*(y2-y1) - (y1-y0)*(x2-x1) int crossProduct = x0*(y1-y2) - y0*(x1-x2)
		 * + (y2*x1-y1*x2); if (crossProduct != 0) nCorners++; sumdx += Math.abs(x2-x1);
		 * sumdy += Math.abs(y2-y1); x0 = x1; y0 = y1; x1 = x2; y1 = y2; } double
		 * pw=1.0, ph=1.0; //pixel width & height (in scaled units) if (imp!=null) {
		 * Calibration cal = imp.getCalibration(); pw = cal.pixelWidth; ph =
		 * cal.pixelHeight; } return sumdx*pw + sumdy*ph - 0.5*nCorners*((pw+ph) -
		 * Math.sqrt(pw*pw+ph*ph));
		 */
	}
	...
	double getLength(ImagePlus imp) {
		...
		if (nPoints > 2) {
			if (type == RoiType.FREEROI.id())
				return getSmoothedPerimeter(imp);
			else if (type == RoiType.FREELINE.id() && !(width == 0 || height == 0))
	...
	static double getLength(int[] xpoints, int[] ypoints, int npoints, boolean closeShape, ImagePlus imp) {
		...
		for (int i = 0; i < npoints - 1; i++)
			length += Math.sqrt(
					sqr((xpoints[i + 1] - xpoints[i]) * pixelWidth) + sqr((ypoints[i + 1] - ypoints[i]) * pixelHeight));
		if (closeShape)
	...
	public FloatPolygon getInterpolatedPolygon(double interval, boolean smooth) {
		...
		if (smooth && (type == RoiType.TRACED_ROI.id() || type == RoiType.FREEROI.id() || type == RoiType.FREELINE.id())) {
	...
	protected int clipRectMargin() {
		return type == RoiType.POINT.id() ? 4 : 0;
	}
