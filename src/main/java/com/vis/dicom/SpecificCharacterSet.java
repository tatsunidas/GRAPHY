    ...
    private static String[] checkISO2022(String[] codes) {
        ...
        for (int i = 0; i < codes.length; i++) {
            ...
            if (code != null && !code.isEmpty() && !code.startsWith("ISO 2022")) {
                ...
                Log.logger.log(Level.WARNING, "Invalid Specific Character Set: [{0}] - treat as [{1}]",
                        new String[] {StringUtils.concat(codes, '\\'), StringUtils.maskNull(codes[0], "")});
                return new String[]{codes[0]};
            }
        }
        if (codes != results) {
            Log.logger.log(Level.WARNING,"Invalid Specific Character Set: [{0}] - treat as [{1}]",
                    new String[] {StringUtils.concat(codes, '\\'), StringUtils.concat(results, '\\')});
        }
    ...
    private static String[] ensureFirstContainsASCII(String[] codes) {
        for (int i = 0; i < codes.length; i++) {
            if (Codec.forCode(codes[i]).containsASCII()) {
                ...
                clone[i] = codes[0];
                Log.logger.log(Level.WARNING,"Invalid Specific Character Set: [{0}] - treat as [{1}]",
                        new String[] {StringUtils.concat(codes, '\\'), StringUtils.concat(clone, '\\')});
                return clone;
            }
        }
        ...
        Log.logger.log(Level.WARNING,"Invalid Specific Character Set: [{0}] - treat as [{1}]",
                new String[] {StringUtils.concat(codes, '\\'), StringUtils.concat(withASCII, '\\')});
        return withASCII;
    }

    //tk
    public String[] toDicomCodes () {
        return dicomCodes;
    }
