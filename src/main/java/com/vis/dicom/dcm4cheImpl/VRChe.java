 ...
 * @author tatsunidas
 */
public enum VRChe implements VR {
	/**
	 * Application Entity
	 */
	...
	VRChe(int code, int headerLength, int paddingByte, ValueType valueType, boolean inlineBinary) {
		this.code = code;
	...
	private static int indexOf(VRChe vr) {
		return vr.code - AE.code;
	}

	private static final VRChe[] VALUE_OF = new VRChe[indexOf(UV) + 1];
	static {
		for (VRChe vr : VRChe.values())
			VALUE_OF[indexOf(vr)] = vr;
	}

	public static VRChe valueOf(int code) {
		try {
			VRChe vr = VALUE_OF[code - AE.code];
			if (vr != null)
				return vr;
	...
	private org.dcm4che3.data.SpecificCharacterSet convertSpecificCharSet(com.vis.dicom.SpecificCharacterSet cs){
		if(cs != null) {
			String[] codes = cs.toDicomCodes();
			org.dcm4che3.data.SpecificCharacterSet sCharSet = org.dcm4che3.data.SpecificCharacterSet.valueOf(codes);
			return sCharSet;
		}else {
			return org.dcm4che3.data.SpecificCharacterSet.getDefaultCharacterSet();
		}
	}
	
	private org.dcm4che3.data.DatePrecision convertDatePrecision(com.vis.dicom.DatePrecision prec){
		if(prec != null) {
			org.dcm4che3.data.DatePrecision dp = new DatePrecision(prec.lastField, prec.includeTimezone);
			return dp;
		}else {
			return new DatePrecision();
		}
	}
	
	private org.dcm4che3.data.DatePrecisions convertDatePrecisions(com.vis.dicom.DatePrecisions precs){
		if(precs != null && precs.precisions != null) {
			org.dcm4che3.data.DatePrecisions dps = new DatePrecisions();
			org.dcm4che3.data.DatePrecision[] dps_context = new DatePrecision[precs.precisions.length];
			int i = 0;
			for(com.vis.dicom.DatePrecision dp:precs.precisions) {
				org.dcm4che3.data.DatePrecision dp_che = new DatePrecision(dp.lastField, dp.includeTimezone);
				dps_context[i++] = dp_che;
			}
			dps.precisions = dps_context;
			return dps;
		}else {
			return new DatePrecisions();
		}
	}

	@Override
	public byte[] toBytes(Object val, com.vis.dicom.SpecificCharacterSet cs) {
			return toBytes(val, convertSpecificCharSet(cs));
	}

	@Override
	public Object toStrings(Object val, boolean bigEndian, com.vis.dicom.SpecificCharacterSet cs) {
		return toStrings(val, bigEndian, convertSpecificCharSet(cs));
	}

	@Override
	public Date toDate(Object val, TimeZone tz, int valueIndex, boolean ceil, Date defVal,
			com.vis.dicom.DatePrecision precision) {
		return toDate(val, tz, valueIndex, ceil, defVal, convertDatePrecision(precision));
	}

	@Override
	public Date[] toDates(Object val, TimeZone tz, boolean ceil, com.vis.dicom.DatePrecisions precisions) {
		return toDates(val, tz, ceil, convertDatePrecisions(precisions));
	}

	@Override
	public Object toValue(Date[] ds, TimeZone tz, com.vis.dicom.DatePrecision precision) {
		return toValue(ds, tz, convertDatePrecision(precision));
	}

	@Override
	public boolean prompt(Object val, boolean bigEndian, com.vis.dicom.SpecificCharacterSet cs, int maxChars,
			StringBuilder sb) {
		prompt(val, bigEndian, convertSpecificCharSet(cs), maxChars, sb);
		return false;
	}
}
