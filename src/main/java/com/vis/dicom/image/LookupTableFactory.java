...
public class LookupTableFactory {
    ...
    private static final UID[] XA_XRF_CUIDS = {
    ...
    public static boolean applyModalityLUT(DicomObject attrs) {
        return !(StringUtils.contains(XA_XRF_CUIDS, attrs.getString(Tag.SOP​Class​UID))
                && StringUtils.contains(LOG_DISP, attrs.getString(Tag.Pixel​Intensity​Relationship)));
    }

    public void setModalityLUT(DicomObject attrs) {
        rescaleIntercept = attrs.getFloat(Tag.Rescale​Intercept, 0);
        rescaleSlope = attrs.getFloat(Tag.Rescale​Slope, 1);
        modalityLUT = createLUT(storedValue,
                (DicomObject)attrs.getNestedDataset(Tag.Modality​LUT​Sequence));
    }

    public void setPresentationLUT(DicomObject attrs) {
        setPresentationLUT(attrs, false);
    }

    public void setPresentationLUT(DicomObject attrs, boolean ignorePresentationLUTShape) {
        DicomObject pLUT = (DicomObject)attrs.getNestedDataset(Tag.Presentation​LUT​Sequence);
        if (pLUT != null) {
            int[] desc = pLUT.getInts(Tag.LUT​Descriptor);
            if (desc != null && desc.length == 3) {
                int len = desc[0] == 0 ? 0x10000 : desc[0];
                presentationLUT = createLUT(new StoredValue.Unsigned(log2(len)), 
                        resetOffset(desc), 
                        pLUT.getSafeBytes(Tag.LUT​Data), pLUT.bigEndian());
            }
        } else {
            String pShape;
            inverse = (ignorePresentationLUTShape || (pShape = attrs.getString(Tag.Presentation​LUT​Shape)) == null
                    ? "MONOCHROME1".equals(attrs.getString(Tag.Photometric​Interpretation))
    ...
    public void setVOI(DicomObject img, int windowIndex, int voiLUTIndex,
            boolean preferWindow) {
        if (img == null)
            return;

        DicomObject vLUT = (DicomObject)img.getNestedDataset(Tag.VOILUT​Sequence, voiLUTIndex);
        if (preferWindow || vLUT == null) {
            float[] wcs = img.getFloats(Tag.Window​Center);
            float[] wws = img.getFloats(Tag.Window​Width);
    ...
    private void adjustVOILUTDescriptor(DicomObject vLUT) {
        int[] desc = vLUT.getInts(Tag.LUT​Descriptor);
        byte[] data;
        if (desc != null && desc.length == 3 && desc[2] == 16
                && (data = vLUT.getSafeBytes(Tag.LUT​Data)) != null) {
            int hiByte = 0;
            for (int i = vLUT.bigEndian() ? 0 : 1; i < data.length; i++, i++)
                hiByte |= data[i];
            if ((hiByte & 0x80) == 0) {
                desc[2] = 40 - Integer.numberOfLeadingZeros(hiByte & 0xFF);
                vLUT.setInt(Tag.LUT​Descriptor, VR.SS, desc);
            }
        }
    }

    private LookupTable createLUT(StoredValue inBits, DicomObject attrs) {
        if (attrs == null)
            return null;

        return createLUT(inBits, attrs.getInts(Tag.LUT​Descriptor),
                attrs.getSafeBytes(Tag.LUT​Data), attrs.bigEndian());
    }
    ...
    public boolean autoWindowing(DicomObject img, Raster raster) {
        return autoWindowing(img, raster, false);
    }

    public boolean autoWindowing(DicomObject img, Raster raster, boolean addAutoWindow) {
        ...
        if (addAutoWindow) {
            img.setFloat(Tag.Window​Center, VR.DS, windowCenter);
            img.setFloat(Tag.Window​Width, VR.DS, windowWidth);
        }
        return true;
    }
